...
export function renderMarkdownDoc(raw: string): RenderedDoc {
...
export function renderJsonDoc(raw: string, slug: string[]): RenderedDoc {
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch (error) {
    return {
      html: `<h1 class="text-3xl font-semibold tracking-tight mb-6">Document</h1><p>${getFriendlyErrorMessage(error)}</p>`,
      toc: [],
    };
  }
